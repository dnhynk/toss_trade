...
> ### ★ 2026-08-28 — **매매 논제를 닫았다** (사용자 결정 D-34 · `docs/80`)
>
> 몰림은 실재한다(토스 점유율 $0~2 정규장 52% · 데이 95%). 그 폭도 실재한다(토스 상위 20 · $0~5 가
> 데이→프리 전환에서 **−5.1%/일, 6/6**). **그런데 파는 쪽에만 열려 있다** — 롱온리 계좌의 24 시간 주기에
> 양수 구간이 없고, 데이마켓 지정가 공급도 시뮬레이션에서 음수다. **`E` 지명 · G-2/G-3 · D-2 · 검증 카드 ·
> 홀드아웃은 전부 닫혔다.** **수집기는 계속 돈다** — 목적이 *장기 관측*(세션 경계 프리미엄 · 토스 점유율
> 시계열)으로 바뀌었고 축소안은 `USER-INPUT-QUEUE` **D-35** 대기.
> 아래 절들은 **2026-08-13 시점**의 서술이다. 인용 전에 `docs/80` 을 먼저 읽어라.
