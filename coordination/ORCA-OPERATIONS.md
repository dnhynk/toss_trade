...
## 9. `orchestration reply` 도 긴 본문이 잘린다 (2026-08-07 실증)

메모리·§5 에 *"ESC 후엔 큐된 긴 텍스트가 잘릴 수 있으니 `orchestration reply` 가 안전"*
이라고 적혀 있었는데, **`reply` 자체도 잘렸다.**

실제: 코디네이터가 W3 에 약 90줄짜리 지시를 `reply` 로 보냈더니 워커가 받은 것은
**뒤쪽 6줄부터**였다. 워커가 *"앞부분이 잘려서 도착했다, 다시 보내달라"* 고 물어와서 알았다 —
**워커가 안 물었으면 지시의 대부분이 조용히 사라졌을 것이다.**

**대응:**
- **긴 지시를 한 통에 담지 마라.** 30~40줄 안으로 끊어라.
- 정말 길어야 하면 **스펙 파일로 만들어 `task-create --spec` 으로 보내라** — 그 경로는
  잘린 전례가 없다. `reply`/`send` 는 짧은 지시·정정용이다.
- **받았는지 확인해라.** 중요한 지시는 워커에게 *"무엇으로 이해했는지 한 줄로 답하라"* 를
  붙이는 것이 싸다. 이번엔 워커가 스스로 물어봐서 살았다.

> 이 프로젝트의 "조용한 실패" 목록에 하나 더 붙는다 — 이번엔 **지시가 조용히 사라지는** 쪽이다.
