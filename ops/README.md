...
| `watchdog.ps1` | 5분 주기 무인 감시(프로세스·텔레메트리·W4 계약 카운터·디스크·전원·상호 하트비트·**형제 예약작업 결과**) + 자동 재기동. 역할 `-Role sentinel` 은 워치독 자체를 감시 — 하트비트(사이클 시작)와 `watchdog_cycle_done.txt`(사이클 끝) 둘 다 본다. 시작만 하고 끝을 못 내는 워치독은 `watchdog_cycle_incomplete` CRIT (2026-08-28). 판정 근거·문턱은 `docs/11 §14~20`, 로그 패턴 창의 시간 상한과 예약작업 결과 감시는 `docs/51`. |
| `watchdog_selftest.ps1` | 위 스크립트의 샌드박스 자기시험. 진짜 `watchdog.ps1` 을 임시 DataDir 에서 `-DryRunRestart` 로 돌려 경보/재시작 판정을 검증한다. **가동 중 수집기는 건드리지 않는다.** 이 기계의 실제 디스크·예약작업 상태에 물들지 않게 둘 다 시험용 값으로 덮어쓴다. |
| `daily_health.py` | 아침 리포트 생성 + 결번 따라잡기. `--date` 는 **기존 리포트를 덮지 않는다**(거부, 종료코드 4) — 정말 다시 만들려면 `--force`. 정시 경로는 예외(`docs/51` §4). |
| `register_task_scheduler.ps1` | 위 3개를 Windows 작업 스케줄러에 등록/해제/상태조회. **기본은 DryRun.** |
| `dispatch_sweep.py` | 디스패치된 워커를 훑어 **하트비트·브랜치·보고 매니페스트·경과·머지·소유 여섯 축을 대조**한다. 완료 보고를 믿지 않고 커밋을 기계가 확인한다. 판정마다 등급(`ALERT`/`CHECK`/`NOTE`)이 붙어 아침에 한 줄로 갈린다. 읽기 전용(상태 변경 없음). 배경·판정표는 `docs/38_dispatch_sweep.md`. |
...
python -m ops.dispatch_sweep            # 종료코드 0=조치불필요 1=ALERT 있음 2=CHECK(못 잼) 있음
                                        # NOTE 는 절대 1을 안 만든다 — 그게 등급의 요지다
