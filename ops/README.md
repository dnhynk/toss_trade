...
| `watchdog.ps1` | 5분 주기 무인 감시(프로세스·텔레메트리·W4 계약 카운터·디스크·전원·상호 하트비트·**형제 예약작업 결과**) + 자동 재기동. 역할 `-Role sentinel` 은 워치독 자체를 감시. 판정 근거·문턱은 `docs/11 §14~20`, 로그 패턴 창의 시간 상한과 예약작업 결과 감시는 `docs/51`. |
