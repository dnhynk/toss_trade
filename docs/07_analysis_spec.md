...
- **소속 판정은 ms 부등호가 아니라 매매일 date 단위다** (3차 감사 F-1). 일봉 ts 는
  **00:00 ET** 규약이라(docs/06 §2-2) 세션 ms 스팬으로 비교하면 무너진다:
  day 세션이 없는 매매일(한국 휴일 등)에서는 앵커(= pre 시작 04:00 ET)가 일봉 ts 보다
  **뒤**라서 **평가일 자기 일봉**(그날 전체의 종가·거래량 — 명백한 미래)이 "엄격히 과거"
  필터를 통과했다. 이벤트일은 정의상 폭등일이라 자기 거래량이 분모에 들어가면 z 가
  구조적으로 눌린다 — 자기오염의 정확히 그 형태다.
  → 일봉 ts 를 `daily_bar_date()` 로 매매일 date 에 매핑하고 `bar_date < as_of_date` 로 거른다.
- 00:00 ET 는 캘린더가 준 세션 시작에서 역산한다(`et_midnight_ms`) — ET 오프셋을 코드에
  박지 않기 위함이며(계약 C-1) 서머타임도 캘린더가 이미 반영한 값이라 자동으로 따라온다.
- 앵커 모드가 `as_of_mode` 로 드러난다: `date`(안전) / `ms_unanchored_by_date`(캘린더 없이
  raw ms 만 준 구식 경로 — 실규약 일봉을 못 거른다) / `None`(앵커 없음).
  **캘린더를 함께 주면 raw ms 도 date 로 승격**된다.
...
- **주 분석은 `prereg_daily_baseline(df_1d, as_of_date, calendar)` 로 부른다.**
  서명이 (date, calendar) 인 이유는 3차 감사 F-1 권고다 — raw ms 를 받으면 호출자가
  `t0_ms` 를 넘기는 실수를 막을 수 없고, 그러면 **모든 날에서** 평가일 자기 일봉이 통과한다.
...
| 그 매매일에 1분봉 또는 일봉이 없음 | $r$ 계산 불가 → 그 날은 건너뛴다(행 없음). 건수는 `n_r_uncomputable` 로 보고(§7-e 의무) |
| **결측일을 사이에 둔 점프** | **(마지막 관측 매매일, 관측 매매일] 구간의 모든 매매일을 분할일로 등록**한다 — 아래 참조 |
| 1분봉 마지막 종가가 0 | 나눗셈 불가 → 그 날 제외 |
| 일봉 ts 의 매매일 소속 | **date 단위 매핑**(`daily_bar_date`). ms 스팬 버킷팅은 day 세션 없는 날의 일봉을 조용히 버린다(3차 감사 F-2 트리거 (b)) |

**보수적 확대 등록 (3차 감사 F-2)** — 이것이 경계조건의 핵심이다:

점프가 결측 구간을 넘어 관측되면, 관측일만 등록하는 것은 **틀린다**. 분할 유효일 $D$ 의 $r$ 을
계산할 수 없으면 점프는 $D{+}1$ 에서 보이는데, $D{+}1$ 만 등록하면

- $D$: 당일 조건이 제외되지 않는다 → $D{-}1$ 전일종가(분할 전 스케일) 대 $D$ 원주가(분할 후
  스케일) → **+N00% 가짜 `kind='day'` 이벤트가 카탈로그에 잔류**한다.
- $D{+}1$: 억울하게 제외된다(실해는 없다 — $D$ 대비 갭은 ~0).

즉 "결측 건너뛰기"는 분할 **사실**은 잡지만 **보호가 필요한 날을 보호하지 못한다**.
구간 내 어느 날이 진짜 분할일인지는 원리상 알 수 없으므로 **구간 전체를 등록**한다.
넓힌 날 수는 `n_widened`, 넓힘 때문에 늘어난 제외는 `split_excluded`(scope=`symbol_day`)로
드러난다. 결측이 없으면 구간은 관측일 1개뿐이라 기존 동작과 같다.
...
> ### ⚠️ 위 근거는 폐기됐다 — **계약 C-7 개정 A2 (2026-08-09)** 를 보라
>
> ~~근거: $T_0$ 봉의 종가·거래량은 그 분이 **끝난 뒤에만** 관측된다. … 실시간 검출기는
> 봉 종료 시점에 판정하므로 봐도 된다. **랭킹도 같은 규칙을 `snap_ms` 에 적용한다.**~~
>
> **틀린 이유**: 저 문장은 **봉 라벨이 시작 시각**이라는 전제에서 나왔다. 실측으로
> `candles_1m.ts_ms` 는 **종료 시각 라벨**이다(`docs/12` §6.1, 세 갈래 독립 검증 98.9%/99.1%).
> `ts_ms = T` 인 봉은 `[T−60초, T)` 를 담아 **`T` 에 이미 완결**이므로,
> **T0 봉은 실시간 검출기에만이 아니라 누구에게나 `t0` 에 관측 가능하다.**
>
> **★ 특히 마지막 문장이 위험하다.** 랭킹에 같은 규칙을 적용하면 **진짜 룩어헤드가 된다** —
> 랭킹은 구간이 아니라 **순간**이고, W1 실측으로 **도착 시 중앙 16.1초 늙어 있다**(`docs/35`).
> `snap_ms = t0` 인 스냅은 **t0 에 우리 손에 없다.**
>
> **현행 규약 (A2):**
> - **캔들** → `ts_ms <= t0_ms` (관측 가능 컷오프)
> - **랭킹** → **`snap_ms < t0_ms` 엄격. `include_t0` 플래그 대상이 아니다.**
>
> 옛 코드는 **플래그 하나로 둘을 같이 밀었고 그것이 A2 §2 위반**이었다.
> W3 가 실측으로 잡았고(`ranking_snaps_pre` 4.0 vs 6.0, `docs/49`),
> W4 가 검출기는 이미 A2 를 지키고 있음을 확인했다(누출 0, `docs/50`).
> **크기**: 랭킹 컷을 `<=` 로 밀면 채택 스코어가 **+0.140** 움직인다(tier2 게이트 0.35 의 40%).
>
> 이 절을 인용해 코드를 짜지 마라. 정본은 `docs/04` C-7 개정 A2 다.
...
> **모드별로 값이 다르다 — 인용할 때 모드를 반드시 병기하라 (2026-08-12 보강)**
>
> 위 부등식은 **엄격 모드(`strict_lt`)에서 참이다.** "(엄격 모드)" 한정은 원 커밋
> `290f0c8` 부터 있었고, 컷오프 개정(A2) 뒤에 붙은 것이 아니다.
>
> **현행 규약(A2, 캔들 `ts_ms <= t0_ms` = `obs_le`)에서는 `cutoff == t0` 이므로
> `cutoff_lag_min` 은 0 이다** — 실측 60/60 = 0 (`docs/49` §4). 즉 A2 가 없앤 것은
> **구조적 60초 사각**이고, 공백 때문에 0보다 커지는 것은 여전히 가능하다.
>
> **★ 다만 그 실측이 위 문장을 반박하지는 않는다.** 60/60 = 0 은 `obs_le` 값이고
> 위 문장은 `strict_lt` 한정이다. **다른 모드에서 잰 값으로 반박하는 것은
> `docs/48` §6-3 이 금지한 모드 혼합이다.** 한때 세 곳이 이 문언을 *"실측으로 틀린 것이
> 확정됐다"* 고 적었으나 그것은 무효 추론이었다 — W7 이 훑기에서 (다)로 잡았고
> (`docs/48` §11-7 #3), `docs/12` 이력에 2026-08-12 항목으로 정정돼 있다.
>
> 요약: **`strict_lt` 에서는 1 이상(참) · `obs_le` 에서는 0(실측).** 둘 다 참이고 섞으면 안 된다.
...
| 23 | 분할 탐지의 한계 (§2.7) | r 은 두 계열이 **같은 날 모두 존재**할 때만 계산된다 | 결측 구간을 넘은 점프는 **구간 전체를 보수적으로 등록**(3차 감사 F-2). 미계산 건수는 `n_r_uncomputable` 로 보고. 첫 매매일 분할은 원리상 탐지 불가 |
| 24 | 일봉 소속 판정 (§2.1, 3차 감사 F-1) | ~~ms 부등호라 day 세션 없는 매매일에서 평가일 자기 일봉이 베이스라인에 유입~~ | **해소** — date 단위 매핑 + `prereg_daily_baseline` 이 (date, calendar) 서명 |
| 25 | 보수적 확대의 대가 (신규) | 결측 1일마다 분할일이 1일 더 등록돼 그만큼 당일 조건이 제외된다(위양성) | 의도된 트레이드오프 — 놓치면 가짜 갭이 카탈로그에 들어간다. 크기는 `n_widened`·`split_excluded` 로 노출 |
