# 39. `TOP_GAINERS` 를 수집에 추가 — ~~만들었고, **배포는 안 했다**~~ → **배포됐고 수집 중이다**

> **[기록]** 소유 W4 · 2026-08-07 · ⚠ **제목의 취소선 부분이 이 문서의 원래 제목이었다**
> 상태 표기의 뜻과 전수 목록: [`docs/INDEX.md`](INDEX.md)

> ### ★ 정정 (2026-08-13, 문서 정리 세션) — **작성 당일 저녁에 거짓이 됐고 6 일간 안 고쳐졌다**
>
> 제목과 §0 의 *"배포는 안 했다"* 는 **2026-08-07 작성 시점에는 참**이었다.
> 그런데 **같은 날 밤 22:26 KST — 정규장 개장 직전에 배포됐다.**
>
> ```
> $ python -c "sqlite3 'file:.../w5-ops/data/tossmon.db?mode=ro' ..."
> TOP_GAINERS  duration=1d   2,140,182 행
>   최초 수집 2026-08-07 22:26 KST   최종 수집 2026-08-13 12:59 KST
> ```
>
> **원문은 지우지 않았다** — 이 절이 무엇에 대한 반응이었는지가 사라지기 때문이다.
> 아래 §2~ 의 *"남은 것: 재시작"* 류 진술도 같은 이유로 그대로 두었다.
> **전부 "2026-08-07 시점의 상태" 로 읽어라.**
>
> 같은 질의가 D-8(랭킹 4 종 → 2 종 축소)도 확인해 준다 —
> 금액 계열 2 종(`MARKET_TRADING_AMOUNT`·`TOSS_SECURITIES_TRADING_AMOUNT`)은
> **2026-08-04 11:22 KST 를 마지막으로 다시 들어오지 않는다.**
>
> 지금 수집 중인 랭킹은 **거래량 2 종 + `TOP_GAINERS`(1d) = 세 종**이다.
