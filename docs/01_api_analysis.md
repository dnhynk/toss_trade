...
5. ~~미국 시세 실시간/지연 여부 미확인~~ → **실측 완료(W1, 2026-07-30)**: 대형주는 실시간(지연 중앙값 12.8s).
   단 **소형주는 체결이 없으면 `timestamp`가 null 이거나 마지막 체결 시각에 고정**된다 (아래 §6 참조).
6. ~~1분봉 과거 보관 기간 미확인~~ → **실측 완료: 최소 1024일(약 2.8년) 보관**. 2048일은 빈 응답.
   진짜 1분 그리드이며 체결 없는 분은 행 자체가 없다. 소형주도 최소 365일 확인.
   **→ "모니터가 직접 쌓아야 한다"는 전제는 무효. 과거 백필로 라벨링·베이스라인을 즉시 구축 가능.**
7. ~~미국 호가 깊이 미확인~~ → **실측 완료: 미국 `/orderbook`은 최우선 1레벨만** (bids 1, asks 1).
   depth 파라미터 없음. 호가 기반 지표는 스프레드 + bid1/ask1 잔량비로 축소된다.
8. 웹소켓 없음 → 이벤트 감지 지연은 폴링 주기에 종속. 티어링으로 해결.
9. `/trades`는 최근 **최대 50건**만 반환 → 활발한 러너의 테이프는 **표본**이지 전량이 아니다.
   체결 크기 분포는 표본 분포로 해석해야 하며, 폴링 간격이 길면 구간 누락이 발생한다.
10. **수수료 단위가 시장별로 다르다**: `/commissions` 실측 KR `0.00015`(비율=0.015%), US `0.1`(퍼센트=0.1%).
   US 0.1%는 2026년 상시 적용으로 확정됨(이벤트 종료 후 0.25% 인상 계획 철회).
   왕복 0.2% + 환전 스프레드(정규장 우대 ~0.05%, 그 외 ~0.5%) + 슬리피지.

## 5. 미국장 세션 구조 (토스 특유)

토스는 KST 기준 거의 22시간 미국 주식 거래를 제공한다:

> 아래 표는 **W1 라이브 실측(2026-07-30)으로 정정된 값**이다. 스펙 예시값과 다르므로 실측 기준을 따른다.

| 세션 | KST (서머타임 기준) | 특성 |
|---|---|---|
| 데이마켓 | 09:00–**17:00** | 토스 자체 제공(대체거래소 경유). **한국 낮 시간 = 토스 개인 활동 최대 시간대. 유동성 얇음** |
| 프리마켓 | 17:00–22:30 | 미국 프리장. **소형주 timestamp null 다발 구간** |
