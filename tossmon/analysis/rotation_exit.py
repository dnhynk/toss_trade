...
## `rankings_snap.amount_u` 취급 (계약 C-2 개정 / docs/06 §13 — 확정)

`amount_u` 는 **마이크로원(KRW)** 이다. 달러로 읽으면 약 **1,440배 과대**다.
그리고 `amount_u`·`vol_qu` 는 **둘 다 누적이 아니라 롤링 윈도우**(`duration=realtime`,
창 길이 미확정)이므로 **차분을 구간 거래대금/거래량으로 쓰는 것은 무효**다.

이 모듈은 두 규칙을 지킨다:

1. **금액으로 읽지 않는다.** `amount_u` 는 오직 **같은 스냅샷 안의 비율**
   (`amount_share_series`)로만 쓴다 — 환율이 분자·분모에 공통이라 **약분된다.**
   이 성질은 테스트가 강제한다(전 종목 금액에 임의 상수를 곱해도 점유율 불변).
2. **차분하지 않는다.** 구간 거래대금이 필요하면 `vol_qu × last_u` 를 직접 계산해야
   하며(둘 다 USD 계열), 그것도 롤링 창이라 **구간량이 아니다** — 그런 지표는
   이 데이터로 만들 수 없다.
