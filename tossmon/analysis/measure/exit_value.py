...
> ★ **정정 (2026-08-12) — "아직 제대로 재지 않았다" 는 이제 참이 아니다. 쟀고, 결과는
> 음성이다.** 왜 틀렸나: 이 줄은 **작성 시점(2026-08-03)에는 참**이었으나, 이 러너가
> 돌아 산출물이 나온 뒤에도 갱신되지 않았다. 지금 이 줄만 읽는 사람은 **이탈 축을
> 아직 안 판 땅으로** 읽는다 — 실제로는 팠고, 나온 것이 불리한 쪽이다.
> **근거 문서**: `docs/23_shooting_structure` §10-P (이 러너의 산출물 `out/exit_value.json`).
> §10-P.6 판정표에서:
>
> - *"그 여지가 **우리 진입 덕분**인가 → **아니다 — 클레임 철회.** 변동성을 정합하면
>   우위가 0 을 교차하고(+1.07%p [−0.69,+3.06]), 사전 변동성으로 정규화하면 **세 통제군
>   전부** 0 을 교차한다"* (§10-P.7)
> - *"그 여지를 가져오는 규칙이 있나 → **없다** — 관측 가능 **20종 전부** 차이 CI 가 0 교차"*
> - *"사용자의 (다) 감각이 맞았나 → **확인하지 못했다**"*
> - 회수율은 최고가 상한의 **12%** 다(§10-P.2·§10-P.3).
>
> ★ **그러나 아래 두 읽기 중 어느 쪽이 이겼는지는 적지 않는다 — 갈리지 않았기 때문이다.**
> §10-P.6 이 스스로 그렇게 적었다: *"두 읽기 중 어느 쪽인가 — 지금 답은 '아직 모른다'이다.
> 읽기 1('이탈은 별 차이를 못 만든다')을 기각할 근거가 없다. 읽기 2('우리 12종에 정답이
> 없다')도 확인되지 않았다."* 그러므로 **아래 19행의 "이 러너는 어느 쪽인지 판정한다" 는
> 이 표본에서 이행되지 않았다** — 러너는 돌았고 이지선다는 열려 있다.
>
> **여기서 판정하지 않는다** — *"그래서 이탈 축이 죽었다/살았다"* 를 이 정정은 쓰지 않는다.
> 적은 것은 쟀다는 사실, 결과가 있는 곳, 그 결과가 음성이라는 것, 이지선다가 안 갈렸다는
> 것까지다. 원문에 취소선만 긋고 지우지 않은 것은 **작성 시점에는 참**이라서다.
> 지목: `docs/54` §4 (escalation `msg_5a6f50413bb5` · `msg_c1a99fc71cb6`).
