...
import os
CLIPS=[200]; MAX_HOLD=int(os.environ.get('MAX_HOLD_MIN','60'))*60*1000; COMM=0.001
CALM=int(os.environ.get('CALM','0')); NO_THROUGH=int(os.environ.get('NO_THROUGH','0')); PARTIAL=int(os.environ.get('PARTIAL','0'))
print('VARIANT', dict(MAX_HOLD_MIN=MAX_HOLD//60000,CALM=CALM,NO_THROUGH=NO_THROUGH,PARTIAL=PARTIAL))
    ...
    for s0,s1 in zip(starts,ends):
        ...
        for clip in CLIPS:
            ...
            for i,(t,b1,bq,a1,aq) in enumerate(rows):
                ...
                if pos is None:
                    calm_ok = (not CALM) or (i>=5 and all(abs(rows[j][1]-b1)<1e-9 for j in range(i-5,i)) and (a1-b1)>=2*tick(b1)-1e-9)
                    if bid is None or b1>bid["px"]+1e-9:          # (re)join at bid1 -> lose priority
                        bid={"px":b1,"q_ahead":bq,"qty":clip/b1,"vol":0.0,"t":t} if calm_ok else None
                    elif bid is not None:
                        if b1<bid["px"]-1e-9:                        # traded through -> adverse fill
                            if NO_THROUGH: bid=None; continue
                            pos={"px":bid["px"],"qty":bid["qty"],"t":t}; bid=None; buys+=1; continue
                        bid["vol"]+=seg.loc[seg.price_u/1e6<=bid["px"]+1e-9,"qty_u"].sum()/1e6
                        if bid["vol"]>=bid["q_ahead"]+(0 if PARTIAL else bid["qty"]):
                            pos={"px":bid["px"],"qty":bid["qty"],"t":t}; bid=None; buys+=1; continue
                else:
                    ...
                    if ask is None or a1<ask["px"]-1e-9:             # (re)join at ask1 when it comes down to us / first time
                        ...
                        if ask["vol"]>=ask["q_ahead"]+(0 if PARTIAL else ask["qty"]):
                            trades.append(dict(sym=sym,date=date,clip=clip,pb=pos["px"],ps=ask["px"],forced=False,hold_s=(t-pos["t"])/1000)); pos=None; ask=None; continue
